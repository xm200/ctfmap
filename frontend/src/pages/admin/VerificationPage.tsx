...
export function VerificationPage() {
  ...
  return <><PageHeader code="АДМИНИСТРАТОР / ПРОВЕРКА" title="Проверка пользователей" description="Проверка данных участников и управление уровнем доверия." />
    ...
    {!tickets ? <LoadingPanel /> : !visible.length ? <EmptyPanel title="Очередь пуста" text="Для этого статуса заявки отсутствуют." /> : <div className="ticket-list">{visible.map((ticket) => <article className="ticket-card" key={ticket.id}><header><div><span>{ticket.id.toUpperCase()}</span><h2>{ticket.user.username}</h2><p>{ticket.user.email}</p></div><StatusIndicator status={ticket.status} /></header><div className="ticket-grid"><div><span>ОТПРАВЛЕНО</span><b>{new Date(ticket.submittedAt).toLocaleString('ru-RU')}</b></div><div><span>КОНТАКТ</span><b>{ticket.contact}</b></div><div className="ticket-details"><span>ПРЕДОСТАВЛЕННЫЕ ДАННЫЕ</span><p>{ticket.details}</p></div></div><label className="field"><span>ПРИЧИНА / КОНТАКТНЫЕ ДАННЫЕ</span><textarea rows={3} value={comments[ticket.id] ?? ticket.comment ?? ''} onChange={(e) => setComments((current) => ({ ...current, [ticket.id]: e.target.value }))} disabled={ticket.status !== 'pending'} placeholder="Обязательно при отклонении заявки..." /></label>{ticket.status === 'pending' && <footer><button className="button admin-button admin-button--approve" disabled={busy === ticket.id} onClick={() => review(ticket, 'approved')}>ОДОБРИТЬ ✓</button><button className="button admin-button admin-button--reject" disabled={busy === ticket.id} onClick={() => review(ticket, 'rejected')}>ОТКЛОНИТЬ ×</button></footer>}</article>)}</div>}
  </>;
}
