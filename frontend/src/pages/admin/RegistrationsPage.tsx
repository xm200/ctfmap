...
export function RegistrationsPage() {
  ...
  return <><PageHeader code="ADMIN / REGISTRATIONS" title="Event intake queue" description="Модерация заявок организаторов на публикацию соревнований." />
    ...
    {!tickets ? <LoadingPanel /> : !visible.length ? <EmptyPanel title="Очередь пуста" text="Заявки с выбранным статусом отсутствуют." /> : <div className="ticket-list">{visible.map((ticket) => { const externalUrl = safeExternalUrl(ticket.url); return <article className="ticket-card registration-card" key={ticket.id}><header><div><span>{ticket.id.toUpperCase()} // EVENT INTAKE</span><h2>{ticket.title}</h2><p>{ticket.organizer} // {ticket.contact}</p></div><StatusIndicator status={ticket.status} /></header><div className="ticket-grid registration-grid"><div><span>SCHEDULE</span><b>{ticket.startDate} → {ticket.endDate}</b></div><div><span>FORMAT</span><b>{ticket.format.toUpperCase()}</b></div><div><span>LOCATION</span><b>{ticket.city}, {ticket.region}</b></div><div><span>EXTERNAL LINK</span>{externalUrl ? <a href={externalUrl} target="_blank" rel="noopener noreferrer">OPEN SOURCE ↗</a> : <b>INVALID OR INSECURE URL</b>}</div><div className="ticket-details"><span>DESCRIPTION</span><p>{ticket.description}</p></div></div><label className="field"><span>OPERATOR COMMENT</span><textarea rows={3} value={comments[ticket.id] ?? ticket.comment ?? ''} onChange={(e) => setComments((current) => ({ ...current, [ticket.id]: e.target.value }))} disabled={ticket.status !== 'pending'} placeholder="Комментарий организатору..." /></label>{ticket.status === 'pending' && <footer><button className="button admin-button admin-button--approve" disabled={busy === ticket.id} onClick={() => review(ticket, 'approved')}>APPROVE & PUBLISH ✓</button><button className="button admin-button admin-button--reject" disabled={busy === ticket.id} onClick={() => review(ticket, 'rejected')}>REJECT ×</button></footer>}</article>; })}</div>}
  </>;
}
