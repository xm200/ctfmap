...
function validate(form: FormState, today: string): FormErrors {
  ...
  if (form.contact.trim().length < 3) errors.contact = 'Укажите электронную почту, имя в Телеграме или другой контакт.';
  ...
  if (!httpUrl.test(form.url.trim())) errors.url = 'Укажите полный адрес, начинающийся с http:// или https://.';
  if (!httpUrl.test(form.registrationUrl.trim())) errors.registrationUrl = 'Укажите полную ссылку на регистрацию.';
  if (form.ctftimeUrl.trim() && !httpUrl.test(form.ctftimeUrl.trim())) errors.ctftimeUrl = 'Некорректная ссылка CTFтайм.';
  if (form.ctfNewsUrl.trim() && !httpUrl.test(form.ctfNewsUrl.trim())) errors.ctfNewsUrl = 'Некорректная ссылка на «Новости CTF».';
...
export function EventRegistrationPage({ onNavigate }: { onNavigate: (path: string) => void }) {
  ...
  const [form, setForm] = useState<FormState>(() => ({
    ...
    taskCategories: 'Веб, Бинарная эксплуатация, Обратная разработка, Криптография',
    tags: 'Задания, Командное',
  ...
  return <main className="profile-page event-registration-page">
    <div className="noise" />
    <header className="profile-top"><Brand /><div className="profile-top-actions"><button type="button" onClick={() => onNavigate('/profile')}>← ПРОФИЛЬ</button><button type="button" onClick={() => onNavigate('/')}>КАРТА</button></div></header>
    <div className="profile-content event-registration-content">
      <PageHeader code="ПРИЁМ ЗАЯВОК // НОВАЯ ЗАЯВКА" title="Регистрация соревнования" description="Заполните карточку события. После отправки заявка поступит на модерацию." />
      {ticketId ? <section className="submission-success hud-panel">
        <span>ЗАЯВКА ПРИНЯТА</span><h2>Заявка отправлена</h2><p>Заявка <b>{ticketId.toUpperCase()}</b> создана и ожидает проверки администратора.</p><div><button className="button admin-button" type="button" onClick={() => onNavigate('/')}>ВЕРНУТЬСЯ К КАРТЕ</button><button className="button admin-button admin-button--primary" type="button" onClick={() => onNavigate('/profile')}>ВЕРНУТЬСЯ В ПРОФИЛЬ →</button></div>
        ...
        <FormSection code="01" title="Основные сведения" hint="ПУБЛИЧНЫЕ ДАННЫЕ">
          ...
          <Field name="shortTitle" label="КОРОТКОЕ НАЗВАНИЕ" error={errors.shortTitle}><input value={form.shortTitle} onChange={(e) => set('shortTitle', e.target.value)} maxLength={40} placeholder="СИБ CTF 2026" required /></Field>
          ...
          <Field name="contact" label="КОНТАКТ ОРГАНИЗАТОРА" error={errors.contact}><input value={form.contact} onChange={(e) => set('contact', e.target.value)} maxLength={160} placeholder="Электронная почта или имя в Телеграме" required /></Field>
        ...
        <FormSection code="02" title="Формат и даты" hint="КЛАССИФИКАЦИЯ">
          <Field name="format" label="ФОРМАТ" error={errors.format}><select value={form.format} onChange={(e) => set('format', e.target.value as EventFormat)}><option value="online">ОНЛАЙН</option><option value="offline">ОЧНО</option><option value="hybrid">ГИБРИД</option></select></Field>
          <Field name="category" label="КАТЕГОРИЯ" error={errors.category}><select value={form.category} onChange={(e) => set('category', e.target.value as EventCategory)}><option value="training">ТРЕНИРОВКА</option><option value="local">ЛОКАЛЬНОЕ</option><option value="elite">ПРОФЕССИОНАЛЬНОЕ</option></select></Field>
          ...
          <Field name="region" label="РЕГИОН" error={errors.region}><select value={form.region} onChange={(e) => set('region', e.target.value)} required><option value="">ВЫБЕРИТЕ РЕГИОН</option>{regions.map(([id, name]) => <option key={id} value={id}>{name}</option>)}</select></Field>
          <Field name="city" label="ГОРОД" error={errors.city}><input value={form.city} onChange={(e) => set('city', e.target.value)} maxLength={128} placeholder={form.format === 'online' ? 'Необязательно для онлайн' : 'Обязательно'} /></Field>
        </FormSection>

        <FormSection code="03" title="Ссылки" hint="ВНЕШНИЕ ИСТОЧНИКИ">
          ...
          <Field name="ctftimeUrl" label="ССЫЛКА CTFТАЙМ" error={errors.ctftimeUrl}><input type="url" value={form.ctftimeUrl} onChange={(e) => set('ctftimeUrl', e.target.value)} maxLength={500} placeholder="Необязательно" /></Field>
          <Field name="ctfNewsUrl" label="ССЫЛКА НА «НОВОСТИ CTF»" error={errors.ctfNewsUrl}><input type="url" value={form.ctfNewsUrl} onChange={(e) => set('ctfNewsUrl', e.target.value)} maxLength={500} placeholder="Необязательно" /></Field>
        </FormSection>

        <FormSection code="04" title="Задания и участие" hint="СОДЕРЖАНИЕ СОРЕВНОВАНИЯ">
          <Field name="taskCategories" label="КАТЕГОРИИ ЗАДАНИЙ" error={errors.taskCategories} wide><input value={form.taskCategories} onChange={(e) => set('taskCategories', e.target.value)} maxLength={500} placeholder="Веб, Бинарная эксплуатация, Обратная разработка, Криптография" required /></Field>
          <Field name="tags" label="ТЕГИ" error={errors.tags} wide><input value={form.tags} onChange={(e) => set('tags', e.target.value)} maxLength={500} placeholder="Задания, Студенческое, Командное" required /></Field>
        ...
        <footer className="form-actions event-registration-actions"><p>Отправляя заявку, вы подтверждаете достоверность данных. Публикация выполняется только после модерации.</p><button type="button" className="button admin-button" onClick={() => onNavigate('/profile')}>ОТМЕНА</button><button className="button admin-button admin-button--primary" disabled={loading}>{loading ? 'ОТПРАВКА...' : 'ОТПРАВИТЬ ЗАЯВКУ'} →</button></footer>
      </form>}
    </div>
  </main>;
}
