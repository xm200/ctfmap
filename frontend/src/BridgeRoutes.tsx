...
/** Merge-friendly route fragment for auth, self-service profile, and admin nodes. */
export function BridgeRoutes({ path, navigate }: BridgeRoutesProps) {
  ...
  }, [auth.isAuthenticated, auth.isLoading, navigate, requiresAuth]);
  ...
  return <AdminLayout path={normalizedPath} user={currentUser} onNavigate={navigate}
    onLogout={() => { void auth.logout().then(() => navigate('/login')); }}>{page}</AdminLayout>;
}
